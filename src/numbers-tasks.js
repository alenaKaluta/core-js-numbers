...
function getSumOfDigits(num) {
  ...
  const result = numToArray.reduce(
...
function isPowerOfTwo(num) {
  let result;
  const powerNumber = Math.log2(num);
  if (Math.ceil(powerNumber) === Math.floor(powerNumber)) {
    result = true;
  } else {
    result = false;
  }
 ...
 * '5'      => false
 */
function isNumber(numberGiven) {
  return Number.isFinite(numberGiven);
}
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = x1 * 100 + x2 * 100 + x3 * 100;
